...
for C in C_values:
    ...
    clf = SVC(C=C, kernel='rbf', gamma='scale',random_state = 0) 
    clf.fit(X_trn, y_trn)
    models[C] = clf
...
for i in valErr:
    if valErr[i] < minErr: 
        ...
        bestC.append(i)
print("List of Best C values:",bestC)
minErr = 1
finalC = -1
...
print("Best C value:", finalC)
print("The accuracy:", (1-tstErr[finalC])*100,"%")


# Discussion:
# As we can see in the plot, the training error decreases as the C value increases whereas the validation error decreases until the C value is 100. Large C values have slower margin and smaller C values have larger margin. In the above plot, we can see that the large C value has lower training error.
...
for i in valErr:
    if valErr[i] < minErr: 
        ...
        bestG.append(i)
print("List of Best G values :",bestG)
minErr = 1
finalG = -1
...
print("Best G value is:", finalG)
print("The accuracy is:", (1-tstErr[finalG])*100,"%")


# Discussion: As we can see in the plot, as G value increases the training error decreases while as G value increases validation error decreases until G = 1 and then the value increases as G increases. Smaller G value means the model has broad decision region.
# 
...
print("Best Value for G :", finalCG[1])
print("The accuracy :", (1-tstErr[finalCG[0],finalCG[1]])*100,"%")
...
plt.plot(valErr.keys(), valErr.values(), marker='o', linewidth=3, markersize=12) 
plt.plot(trnErr.keys(), trnErr.values(), marker='s', linewidth=3, markersize =12) 
plt.xticks(list(trnErr.keys()), ( '1', '5', '11', '15','21'),fontsize=12)
...
print("Best K value :", finalK )
print("The test accuracy:",(1-tstErr[finalK])*100, "%")


# **Discussion**: Which of these two approaches, SVMs or kNN, would you prefer for this classification task? Explain.
# Answer : kNN is better than SVMs, because kNN gas given an accuracy of 97.3% while svm has given an accuracy of 96.5%. 
